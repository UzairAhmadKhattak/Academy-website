...
def teacher_profile(request):
    ...
    marked_attendance = result = (student_attendance.objects.values('date',"status").annotate(status_count=Count('date')).order_by())  
    ...
    for data in marked_attendance:
        if data["status_count"] == total_students and data["status"] =="Present":
            ...
            if date != today_date:
                ...
                edit_forbiden = ""
            attendance_rows.append([date,total_students,total_present,total_absent,edit_forbiden])
...
def attendance_page(request):
    attendance_results_from_db = student_attendance.objects.filter(date=str(today_date))
    if len(attendance_results_from_db) == 0:
        ...
        <h1/>
        <a class ="btn btn-info" role = "button" href = "http://127.0.0.1:8000/alburhan/user_profile/">User profile<a/>
        <div/>
        """)
...
def submit_attend(request):
    ...
    if request.method == "POST":
        ...
        today_data_student_table = student_attendance.objects.filter(date = today_date)
        ...
        if len(today_data_student_table) == 0:
            ...
            with transaction.atomic(): 
                for student_data_item,status in zip(student_obj,student_presnt_or_absent):
                    student_attendance_obj = student_attendance(fk_student = student_data_item,status = status,date = today_date)
...
def edit_today_attendance(request):
    ...
    attendance_results_from_db = student_attendance.objects.filter(date=str(today_date))
    try:
        previse_attendace = [submitted_attendance .status for submitted_attendance in attendance_results_from_db]
    except:
        previse_attendace = ["empty" for _ in range(len(student_obj))]
...
def resgister(request):
    ...
    if request.method == "POST":
        ...
        # if password1 == password2 and len(password1)>=8 and validating_password(password1):
        if password1 == password2 and len(password1)>=2:
            ...
            if User.objects.filter(username=user_name).exists():
                ...
                user = User.objects.create_user(username=user_name, first_name=user_name,password=password1, email=email)
                user.save()
                messages.success(request,'User Registered')
                last_user_id = int(list(User.objects.all())[-1].id)
                teacher_role = teacher.objects.create(teacher_responsiblity = "Usthaz",fk_teacher_id = last_user_id)
                teacher_role.save()
